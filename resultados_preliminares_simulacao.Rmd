...
Foram gerados tempos de sobrevivência de dois grupos distintos, simulando o comportamento das curvas de sobrevivência estimadas via Kaplan-Meier para pacientes do conjunto de dados original que estavam nos estadiamentos clínicos I e II. Os dados de ambos os grupos foram simulados a partir de distribuições Weibull, cujos parâmetros são descritos abaixo:

- **Para as observações com preditor 0** (simulando pacientes que estavam no estadiamento clínico I), foram gerados dados da distribuição Weibull com parâmetros `shape = 0,48414` e `scale = 164,92854` (correspondentes a uma distribuição Weibull com mediana de 77,360 e valor esperado de 350,710);
- **Para as observações com preditor 1** (simulando pacientes que estavam no estadiamento clínico II), foram gerados dados da distribuição Weibull com parâmetros `shape = 0,5057387` e `scale = 45,2447631` (correspondentes a uma distribuição Weibull com mediana de 21,919 e valor esperado de 88,623).
...
Foram escolhidos esses dois grupos pois eles garantiam que os limites superiores das distribuições uniformes ligadas ao tempo de censura fossem maiores do que 5 ("anos") em todos os cenários considerados.

### Geração dos tempos de censura

Para a obtenção dos tempos de censura, foram geradas observações de variáveis aleatórias $C_i$ com distribuição uniforme no intervalo $[0, \theta_i]$, $i = 0, 1$, sendo $\theta_i$ o valor de $\theta$ que resolvesse a equação $p_c = P(T_i > C_i)$, onde $p_c$ é a probabilidade de censura e $T_i$ é o tempo de sobrevivência latente - que segue a distribuição Weibull cujos parâmetros são especificados acima.
...
tabela_df <- data.frame(
  "Theta / Prob. de censura" = c("Relacionado ao preditor 0", "Relacionado ao preditor 1"),
  "0,25" = c("908,873", "242,288"),
  "0,4"  = c("354,887", "97,841"),
  "0,5"  = c("192,396", "54,306"),
  "0,6"  = c("99,668", "28,878"),
  "0,7"  = c("46,437", "13,880"),
  check.names = FALSE
)
...
Em todas as amostras simuladas, 60% das observações foram geradas com preditor 1 e 40% com preditor 0 (mantendo, aproximadamente, a proporcionalidade observada entre os grupos I e II da variável ECGRUP no conjunto de dados original).
